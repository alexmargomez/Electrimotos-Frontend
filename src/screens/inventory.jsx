...
const Inventory = () => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-gray-200">
      ...
      <div
        className={`m-4 h-full bg-white rounded-sm shadow-xl border-t-3 border-[#FFD700] ${
          showModal ? "animate-border" : ""
        } flex`}
      >
        <div className="flex flex-col w-full h-full">
          <section className="p-4 h-20% shadow-xl">
            ...
            <form onSubmit={handleAddProducts} className="space-x-4 flex">
              ...
              <button
                type="submit"
                className="blue text-white px-4 py-2 rounded"
              >
                Guardar
              </button>
            </form>
            ...
            <div>
              {products.length === 0 ? (
                ...
                products.map((product) => (
                  <div
                    key={product.id}
                    className="grid grid-cols-10 gap-4 p-2 border-t-1 justify-center items-center"
                  >
